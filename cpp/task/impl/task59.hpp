...
#ifndef LEETCODE_TASK59_HPP
#define LEETCODE_TASK59_HPP
...
void nextIndex(const std::vector<std::vector<int> > &res, int &i, int &j, Direction &dir, bool changeDirection) {
    if (changeDirection) {
...
std::vector<std::vector<int>> generateMatrix1(int n) {
...
std::vector<std::vector<int>> generateMatrix2(int n) {
    auto result = std::vector<std::vector<int>>(n, std::vector<int>(n, 0));
    int loop = n / 2;
