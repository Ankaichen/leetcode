...
namespace TypeTraits {
    ...
    template<typename T> requires container_c<T>
    ...
    template<typename T> requires is_any_task_c<T>
    ...
    template<typename T> requires is_any_test_case_reader_c<T>
    class is_any_test_case_reader<T> : public std::true_type {
    };

    template<typename T>
    class is_test_case_reader
            : public std::conjunction<is_any_test_case_reader<T>, std::negation<std::is_abstract<T>>> {
    };
    ...
    template<typename T> requires is_any_test_result_processor_c<T>
    class is_any_test_result_processor<T> : public std::true_type {
    };

    template<typename T>
    class is_test_result_processor
            : public std::conjunction<is_any_test_result_processor<T>, std::negation<std::is_abstract<T>>> {
    };
    ...
    private:
        ...
        static std::true_type _helper(const Task<Name, R, P, Res(Args...)> *, const TestCaseReader<Res(Args...)> *,
                                      const TestResultProcessor<Res, Any> *) { return {}; }

        static std::false_type _helper(...) { return {}; }

    public:
        static constexpr bool value = decltype(_helper(std::declval<T *>(), std::declval<R *>(),
                                                       std::declval<P *>()))::value;
    };
    ...
    template<is_any_task_c T, is_test_case_reader_c R, is_test_result_processor_c P> requires is_task_fit_c<T, R, P>
