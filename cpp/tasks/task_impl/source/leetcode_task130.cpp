...
static void solve1(std::vector<std::vector<char>>& board) {
    ...
    for (int i = 0; i < m; ++i) {
        for (int j = 0; j < n; ++j) {
            ...
            if (i + 1 < m && board[i + 1][j] == 'O') uf.union_value((i + 1) * n + j, i * n + j);
            if (j + 1 < n && board[i][j + 1] == 'O') uf.union_value(i * n + j + 1, i * n + j);
        }
    }
    for (int i = 0; i < m - 1; ++i) {
        for (int j = 0; j < n - 1; ++j) {
...
static void dfs(const std::vector<std::vector<char>>& board, int starti, int startj, std::vector<std::vector<bool>>& flags) {
    flags[starti][startj] = true;
    std::size_t size = board.size();
    int upi = starti - 1, downi = starti + 1, leftj = startj - 1, rightj = startj + 1;
