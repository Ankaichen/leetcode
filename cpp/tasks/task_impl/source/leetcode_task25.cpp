...
static ListNode *reverseKGroup2(ListNode *head, int k) {
    ...
    auto reverse = [](ListNode *head, int k) -> ListNode * {
    ...
    ListNode tempHead(0, head), *p = &tempHead, *q;
    while ((q = seek(p, k)) != nullptr) {
        ListNode *next = q->next;
        (p = reverse(p, k))->next = next;
    }
    return tempHead.next;
}

ListNode *LeetcodeTask25::solve(ListNode *head, int k) const { return reverseKGroup2(head, k); }
